...
-- Richest Celebrity --
SELECT c.celebrity_id, p.popular_character_name, c.net_worth
FROM celebrity  AS c
INNER JOIN performance AS p
...
-- Net Worth Topper --
SELECT country, SUM(net_worth) AS average_net_worth
FROM celebrity
GROUP BY country
...
SELECT p.popular_character_name, SUM(c.net_worth) AS total_net_worth
FROM celebrity AS c
INNER JOIN performance AS p 
...
SELECT gender, ROUND(SUM(net_worth),2) AS average_net_worth
FROM celebrity
GROUP BY gender
ORDER BY average_net_worth DESC;

-- Average Net Worth By Country --
SELECT country, ROUND(SUM(net_worth),2) AS average_net_worth
FROM celebrity
GROUP BY country
ORDER BY average_net_worth DESC;

 -- Least Wealthy Celebrities--
SELECT p.popular_character_name, SUM(c.net_worth) AS average_net_worth
FROM celebrity AS c
INNER JOIN performance AS p 
ON c.celebrity_id = p.celebrity_id
GROUP BY p.popular_character_name
